import {
  Rule, Tree, SchematicsException} from '@angular-devkit/schematics';
...
export function updateHTML(options: MakeStaticOptions): Rule {
  return async (tree: Tree) => {
    ...
    // Reading config file (config .json file extract by ui-builder)
    const configFileContent = await host.readFile(options.config);
    ...
    if (options.createBase64Images) {
      const assetsFolderPath = normalize(`${project.sourceRoot}/assets`);
      if (!host.isDirectory(assetsFolderPath)) {
        console.warn(`Directory ${assetsFolderPath} does not exist: could not generate base64 image`);
      }
      else {
        await createBase64Images(config, tree, assetsFolderPath);
      }
    }

    // Inferring the project app folder
    const srcFolderPath = normalize(`${project.sourceRoot}/app`);
    if (!host.isDirectory(srcFolderPath)) {
      throw new SchematicsException(`Directory ${srcFolderPath} does not exist`);
    }

    // Searching for HTML files to transform in the project app
    const htmlTemplates = findHtmlTemplates(tree, srcFolderPath);
    if (htmlTemplates.length === 0) {
      console.warn(`No HTML template could be found under ${srcFolderPath}`);
    }
    ...
    if (options.commentScssImport) {
      ...
      if (tree.exists(scssPath)) {
        ...
        await commentScssImport(host, scssPath);
      }
      else {
...
async function commentScssImport(host: workspaces.WorkspaceHost, scssPath: string) {
