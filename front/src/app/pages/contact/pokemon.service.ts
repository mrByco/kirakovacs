...
export class PokemonService {
  ...
  async getPokemon() {
    ...
    const pokemonPromises = pokemon.results.map((p: any) => this.createPokemon(p));
  ...
  async createPokemon(p: { name: string }): Promise<Pokemon> {
    ...
    return {
      id: poke.id,
      name: p.name,
      description: desc.flavor_text_entries
        .filter(({ language, version }: { language: any, version: any }) => language.name === 'en' && version.name === 'x')
        .map(({ flavor_text }: { flavor_text: string }) => flavor_text)
        .join().replaceAll("", " "),
      abilities: poke.abilities.map(({ ability }: { ability: { name: string } }) => ability.name),
      weight: poke.weight,
